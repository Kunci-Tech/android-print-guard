...
export function synthesizeCaptures(
  ...
  for (const capture of sorted) {
    ...
    if (parsed.isTestPrint) {
      ...
      parsed.department === 'BAR' ||
      parsed.department === 'HOT KITCHEN' ||
      parsed.department === 'COLD KITCHEN' ||
      parsed.department === 'KITCHEN' ||
      parsed.department === 'CAPTAIN ORDER' ||
      parsed.department === 'CHECKER' ||
      parsed.asciiText.includes('** ADD ITEM **') ||
      parsed.asciiText.includes('** VOID ITEM **') ||
      parsed.asciiText.includes('** CAPTAIN ORDER **')
    ) {
      category = 'KITCHEN_TICKET';
    } else if (
      parsed.department === 'MAIN POS BILL' ||
      parsed.asciiText.includes('KUNCI KUPPI') ||
      parsed.asciiText.includes('BUKTI PEMBAYARAN') ||
      parsed.asciiText.includes('Tender')
    ) {
